...
export function Button({ textOnly = false, confirmText = "Confirmar", confirm = false, icon = undefined, text = "button", outline = false, disabled = false, className = "", onClick = (() => { }) }: ButtonProps) {
  ...
  return (
    <div onClick={() => {
      ...
      hover:border-[1px] ring-0  ${awaitingConfirmMode ? "ring-red-300" : 'ring-brand-50'} relative ${disabled ? 'bg-neutral-900' : `${outline ? 'text-zinc-400' : awaitingConfirmMode ? 'bg-red-300 text-white' : 'bg-brand-100 text-white'} 
