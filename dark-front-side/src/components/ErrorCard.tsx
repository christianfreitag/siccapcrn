...
export function ErrorCard({ timer = 0, status = "error", errors = ["Ocorreu um erro"], setErrors }: ButtonProps) {
    ...
    return (errors.length > 0 ? <div className={`h-fit w-[30vw] ${status == "error" ? 'bg-black border-b-[3px] border-red-500' : status == "ok" ? 'bg-black border-b-[3px] border-green-600' : 'bg-black border-b-[3px] border-yellow-500'} text-zinc-400 fixed right-5 bottom-5 z-10 shadow-md rounded-md p-3 flex items-center`}>
        {errors.length == 1 ?

            <div className={'w-[95%] text-zinc-400 flex items-center text-lg'}>
                ...
                {errors[0]}</div> :
            <div className={'w-[95%] text-zinc-400 flex-col'}>
        ...
        <div className={'w-[5%] cursor-pointer'} onClick={() => { setErrors != undefined ? setErrors({ errors: [], status: "error", timer: false }) : null }}><X className="text-zinc-400 p-2 h-8 w-8 hover:text-brand3-100 " /></div>
    </div> : null
    )
