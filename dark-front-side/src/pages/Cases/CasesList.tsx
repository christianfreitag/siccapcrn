  ...
  return (
    <div className={`h-full w-full overflow-hidden  rounded-md pt-6 bg-zinc-800`}>
      ...
      <div className={`h-[6%] min-h-[3rem] w-full mt-5  rounded-md flex bg-black p-1`} onClick={() => {}}>
        ...
        <Select
          placeholder="Status"
          className="z-40 min-w-[10rem] mr-4 flex-1  my-react-select-container my-react-select-container"
        ...
        <div className="rounded-md  min-w-[10rem]  h-10  flex mb-2 ">
          ...
          >
            <CaretLeft className={`${Math.ceil(totalCases / 8) > 0 && page > 1 ? "text-zinc-400" : "text-gray-300 "}`} />
          </div>
          ...
          >
            <CaretRight className={`${Math.ceil(totalCases / 8) > 0 && page < Math.ceil(totalCases / 8) ? "text-zinc-400" : "text-gray-300 "}`} />
          </div>
        </div>
      </div>
      ...
      <div className={`h-auto overflow-y-scroll min-h-fit max-h-[60%] w-full p-1`}>
        {cases.length > 0 ? (
          ...
          <div className="w-full h-20 flex items-center justify-center text-xl text-lightBlack-100">
            <FolderNotchOpen weight="thin" size={"80%"} className={"text-zinc-400 w-fit"} /> <p className="p-5">Nenhum caso encontrado.</p>
          </div>
        )}
      </div>
      <div className="bg-neutral-900 w-full  flex items-center justify-center cursor-pointer hover:bg-opacity-50">Total: {totalCases}</div>
