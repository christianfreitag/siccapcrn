...
export function Case() {
  ...
  return loadPage?(
    ...
    <div className={` w-full  h-full overflow-y-scroll overflow-x-hidden relative p-1`}>
      <div className={`h-[2rem] w-full p-2 mb-8 font-semibold text-3xl text-zinc-400 flex items-baseline mr-10`}>
        Caso
        <span className="text-zinc-400 font-medium text-lg ml-4"> Linha do tempo </span></div>
      {/*@ts-ignore*/}
      ...
      <div className={` bg-zinc-800 ${caseMovements != null && caseMovements?.length <= 0 ? 'h-[5%]  min-h-[7rem] ' : 'h-[20%]  min-h-[8rem] '} w-full  p-2 rounded-lg shadow-sm relative `}>
      ...
      {!end_date ? <div className={` bg-zinc-800  h-fit min-h-[6rem] w-full mt-2  rounded-lg shadow-sm relative p-4`}>
