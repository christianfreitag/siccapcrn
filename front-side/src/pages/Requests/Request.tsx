    ...
    <div className={`h-full w-full overflow-hidden  rounded-md pt-6 bg-white`}>
      ...
      <div className={`h-[6%] min-h-[3rem] w-full flex bg-gray-50 p-1`} onClick={() => {}}>
        ...
        <div className=" flex">
          ...
          >
            <CaretLeft className={`${Math.ceil(totalRequest / 8) > 0 && page > 1 ? "text-brand-100" : "text-gray-300 "}`} />
          </div>
          ...
          >
            <CaretRight className={`${Math.ceil(totalRequest / 8) > 0 && page < Math.ceil(totalRequest / 8) ? "text-brand-100" : "text-gray-300 "}`} />
          </div>
        </div>
      </div>
                  ...
                  <div key={index} className={` pl-2 w-full justify-start  p-[1%] lg:text-[0.92rem]  md:text-sm ${index != requests.length - 1 ? "border-b-[1px]" : ""}`}>
                    <div className={`flex justify-start  `}>
                      ...
                      <span className="flex-1 ">
                        ...
                        <span className="text-gray-500 text-sm ml-2 underline"> mais {value._count.Investigated_requests - 1}</span> : null}
                      </span>
                    </div>
                  </div>
                );
      ...
      <div className="bg-gray-100 w-full  flex items-center justify-center cursor-pointer hover:bg-opacity-50">Total: {totalRequest}</div>
    </div>
  );
}
