    ...
    <div className={`h-full w-full overflow-hidden  rounded-md pt-6 bg-white`}>
      ...
      <div className={`h-[6%] min-h-[3rem] w-full mt-5  rounded-md flex bg-gray-50 p-1`} onClick={() => {}}>
        ...
        <div className="rounded-md  min-w-[10rem]  h-10  flex mb-2 ">
          ...
          >
            <CaretLeft className={`${Math.ceil(totalAnalysts / 8) > 0 && page > 1 ? "text-brand-100" : "text-gray-300 "}`} />
          </div>
          ...
          >
            <CaretRight className={`${Math.ceil(totalAnalysts / 8) > 0 && page < Math.ceil(totalAnalysts / 8) ? "text-brand-100" : "text-gray-300 "}`} />
          </div>
        </div>
      </div>
                  ...
                  <div key={index} className={` pl-2 w-full justify-start  p-[1%] lg:text-[0.92rem]  md:text-sm ${index != analysts.length - 1 ? "border-b-[1px]" : ""}`}>
                    <div className={`flex justify-start cursor-pointer`}>
                      ...
                      <span className="flex-1">
                        <div className={`w-fit bg-gray-100 p-1 rounded-full flex items-center pr-2`}>
                          <div
      ...
      <div className="bg-gray-100 w-full  flex items-center justify-center cursor-pointer hover:bg-opacity-50">Total: {totalAnalysts}</div>
    </div>
  );
}
