...
export function SubSideBar({
  ...
  return (
    <div
      className={`flex-row bg-white overflow-hidden ${isSubSideBarOpen ? "w-60" : "w-0 p-0"
      ...
      <div className="flex-row  w-full h-[90vh]">
        <div className={`flex items-center  pr-2`}><div
          className={`noselect w-full  font-bubblebody text-4xl p-4
           text-brand-100 font-extrabold transition-all duration-500 ${isSubSideBarOpen ? "pl-10" : "pl-4"}`}>delac</div>
        ...
        {<div className={`flex  h-[12%] w-[100%] items-end`}>
          <Popover placement="left-start" >
            <PopoverHandler>
              <div className={`flex items-center hover:bg-brand2-100 w-full cursor-pointer p-2 text-brand-100`}><div className={`rounded-full h-6 w-6 bg-brand-100 ml-3 flex items-center justify-center mr-2 `}><Plus color={"white"} /></div> Novo</div>
            </PopoverHandler>
            <PopoverContent className={`rounded-md h-fit z-50  w-[10rem] shadow-md pt-2 duration-[0] ml-2 border-[1px] pl-2 pr-2`}>
              <div className=" h-3 w-3 rotate-45 translate-x-[-120%] translate-y-[50%] bg-white top-2 border-l-[1px] border-b-[1px] absolute z-auto "></div>
              ...
              return (
                <li key={index} className={`pt-[3%] `}>
                  <div onClick={() => { setItemListOpen(index != itemListOpen ? index : -1) }}
                    className={`noselect font-medium text-base pl-5 cursor-pointer flex justify-between pr-5 text-brand-100`}>
                    {value.title}
                      ...
                      value.subItens.map((sValues, subIndex) => {
                        return <li key={subIndex} onClick={() => {
                          ...
                          className={`noselect items-baseline flex justify-between  text-base pl-10 cursor-pointer transition-all md:pt-[2%] lg:pb-[2%] lg:pt-[3%] w-full hover:bg-brand2-100 ${subSideBarMenuSelected[0] == index &&
                            ...
                            ? "font-semibold duration-75 text-brand-100 bg-slate-100"
